...
void printUsage(const char* program_name) {
    ...
    std::cout << "  --tp_size <num>            Number of TP ranks (decode servers) [default: 1]" << std::endl;
    std::cout << "  --prefill_ip <ip>          Prefill server IP [default: 127.0.0.1]" << std::endl;
    std::cout << "  --prefill_port <port>      Prefill transfer port [default: 60051]" << std::endl;
    std::cout << "  --base_grpc_port <port>    Base gRPC port for decode servers [default: 50151]" << std::endl;
    std::cout << "  --base_transfer_port <port> Base transfer port for decode servers [default: 60151]" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    P2PConnectorPerfTestClientConfig config;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "--prefill_ip" && i + 1 < argc) {
            config.prefill_ip = argv[++i];
        } else if (arg == "--prefill_port" && i + 1 < argc) {
            config.prefill_port = std::atoi(argv[++i]);
    ...
    P2PConnectorPerfTestClient client(config);

    if (!client.init()) {
        std::cerr << "Failed to initialize P2PConnectorPerfTestClient" << std::endl;
        return 1;
    }

    return client.run(g_running);
}
