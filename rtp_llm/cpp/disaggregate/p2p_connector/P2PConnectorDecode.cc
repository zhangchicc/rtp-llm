...
bool P2PConnectorDecode::init() {
    if (gpt_init_parameter_.tp_rank_ == 0) {
        scheduler_ = std::make_shared<P2PConnectorDecodeScheduler>(gpt_init_parameter_, metrics_reporter_);
        if (!scheduler_->init()) {
    ...
    worker_ = std::make_shared<P2PConnectorDecodeWorker>(gpt_init_parameter_, kv_cache_allocator_, metrics_reporter_);
    if (!worker_->init()) {
